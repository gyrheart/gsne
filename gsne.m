...
function [Y,loss] = gsne(X,lambda,varargin)

%GSNE global t-Distributed Stochastic Neighbor Embedding:
%gsne function modifies the MATLAB built-in function tsne by introducing a
%global cost function. The parameter lambda is the weight of the global
%cost function. Lambda = 0 is equal to tsne.
%The other parameters are the same as tsne. For now the
%global cost function is only implemented in the 'exact' algorithm, so the
%'Algorithm' option in the input must be set as 'exact' to make gsne
%effective. The default 'Algorithm' is 'barneshut', which is faster but
%will not implement gsne.
%See more details of gsne algorithm in [1]: 
%[1] Yuansheng Zhou, Tatyana Sharpee. Using global t-SNE to preserve 
%    inter-cluster data structure. BioRxiv 331611; doi: https://doi.org/10.1101/331611

%The paramters of tsne built-in function are shown as below: 
%TSNE t-Distributed Stochastic Neighbor Embedding.
%   Y = gsne(X,lambda) returns the representation of the N by P matrix X in the
...
[Y,loss] = tsneEmbedding(X,ystart,lambda,probMatX,exaggeration,learnrate,...
...
% Implement global t-sne by adding a global cost function
function [grad,probMatY] = tsneGradient(probMatX,X,Y, lambda)
...
% Global probability distribution of X
probMatXGlobal = max(numeratorProbMatXGlobal./sum(numeratorProbMatXGlobal(:)),realmin(class(X)));

N = size(Y,1);
Ysum = sum(Y.^2,2);
numeratorProbMatYGlobal = 1 + bsxfun(@plus,Ysum, bsxfun(@plus,Ysum', -2*(Y*Y'))); 
...
% Global probability distribution of Y
N = size(Y,1);
Ysum = sum(Y.^2,2);
...
% disp(pdiff)
pdiff = numeratorProbMatY.*((probMatX - probMatY)-lambda*(probMatXGlobal - probMatYGlobal));
grad = 4 * (diag(sum(pdiff,1))-pdiff) * Y; 
end

function [Y,loss] = tsneEmbedding(X,Y,lambda,probMatX,exaggeration,...
...
while (iter<=options.MaxIter && ~stop)
    ...
    if strcmpi(algorithm,'exact')
        [grad,probMatY] = tsneGradient(probMatX,X,Y,lambda);
